<template>
  <div>
    ...
    <div id="printMe" style="width: 21cm;height: 29.7cm;margin: 0 auto;">
      <div v-for="(item,index) in this.selectArr" :key="index" style="width:350px;height:300px;float: left;border: 1px solid #1890ff;text-align: center;margin-top: 20px;margin-left: 20px">
        <div style="margin: auto;margin-top: 5px">科室：{{ item.dname }}</div>
        <canvas :id="item.did" style="text-align: center;margin-left: 10px" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    const obj = sessionStorage.getItem('printArr')
    this.selectArr = JSON.parse(obj)
    this.depname = this.selectArr[0].dname
